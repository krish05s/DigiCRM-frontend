...
export default function QuotationPage() {
  ...
  const openQuotationModal = async (lead) => {
    ...
    try {
      ...
      const historyWithFiles = await Promise.all(historyData.map(async (hist) => {
  ...
  const handleEditClick = (item) => {
    ...
    setForm({
      ...
      quotation_date: item.quotation_date ? new Date(item.quotation_date).toISOString().split('T')[0] : new Date().toISOString().split("T")[0],
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredQuotations.length > 0 ? (
                        ...
                        <tr key={q.lead_id} className="border-b border-gray-50 hover:bg-indigo-50/30 transition-colors">
                          ...
                          <td className="text-lg px-3 text-center">
                            {(q.displayStatus === "Won" || q.displayStatus === "Lost") ? (
                              ...
                              <button onClick={() => openQuotationModal(q)} className="w-9 h-9 tracking-widest rounded-full border inline-flex items-center justify-center transition-all hover:bg-green-50 border-green-400 text-green-600 cursor-pointer shadow-sm mx-auto">
                          ...
                          <td className="px-3">
                            <select
                              ...
                              onMouseDown={(e) => {
                                ...
                                // lock pending when not approved OR reverted from Won/Lost
                                if (isPendingLocked) {
                              ...
                              className={`border rounded-sm px-3 py-1 text-xs font-semibold outline-none
                                ...
                                ${q.displayStatus === "Pending" && !q.has_approved
                          ...
                          <td className="px-3 text-center">
                            {q.latest_quotation_id ? (
                              (q.displayStatus === "Won" || q.displayStatus === "Lost") ? (
                                <div className="text-gray-300 w-8 h-8 rounded-full flex items-center justify-center mx-auto" title="Locked">
                                  <i className="bi bi-lock-fill"></i>
                                </div>
                              ) : (
                                // <button onClick={() => openDeleteModal(q.latest_quotation_id)} className="text-gray-400 hover:text-red-600 transition-colors bg-white hover:bg-red-50 w-8 h-8 flex items-center justify-center shadow-sm mx-auto">
                                <button onClick={() => openDeleteModal(q.latest_quotation_id)} className="text-gray-400 hover:text-red-600 cursor-pointer">
                      ...
                      <tr><td colSpan="12" className="text-center py-10 text-gray-400">No Quotations Found</td></tr>
                    )}
                  </tbody>
                </table>
                {/* Pagination */}
            ...
            <div className="flex flex-1 overflow-hidden relative">
              ...
              <div className="w-5/12 bg-white border-r border-gray-100 flex flex-col relative z-10 overflow-y-auto">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        classNames={{
                          control: ({ isFocused }) =>
                            `w-full mt-1 border rounded-lg px-3 py-2 text-sm bg-gray-50 outline-none
                            ${isFocused
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50 mt-auto flex gap-3">
                  ...
                  {editingId && (
                    <button onClick={() => { setEditingId(null); setForm({ quotation_no: form.quotation_no, quotation_date: new Date().toISOString().split("T")[0], activity_type: "", quotation_status: "Pending", assignee: form.assignee, amount: "", discount: "", tax: "0", grand_total: "", description: "" }) }} className="flex-none bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-xl px-4 py-3 text-sm font-semibold transition-all">Cancel</button>
                  )}
                </div>
              </div>
                        ...
                        <div key={index} className={`bg-white border rounded-xl p-4 shadow-sm transition-colors ${item.quotation_status === 'Approved' ? 'border-green-400 bg-green-50/20' : 'border-gray-200 hover:border-blue-200'}`}>
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="flex gap-2 items-center">
                              ...
                              <span className={`text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-md ${item.quotation_status === 'Approved' ? 'bg-green-100 text-green-700' : item.quotation_status === 'Declined' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>{item.quotation_status || "Pending"}</span>
      ...
      {showFileModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-[540px] rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
            ...
            <div className="p-6">
              <div className="w-full border-2 border-dashed border-blue-200 rounded-xl flex flex-col items-center justify-center p-8 bg-blue-50/30 hover:bg-blue-50/70 transition-colors cursor-pointer"
                onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} onClick={() => document.getElementById('quotFiles').click()}
              >
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          ...
          <div className="bg-white w-[420px] rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
            ...
            <div className="flex justify-center gap-3 px-6 py-4 border-t bg-gray-50">
              ...
              <button onClick={() => setShowDeleteModal(false)} className="px-5 py-2 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-100">
                Cancel
              </button>

              <button onClick={handleDeleteQuotation} disabled={isDeleting}
        ...
        <div className="fixed inset-0 bg-gray-900/30  flex items-center justify-center backdrop-blur-sm  z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-80">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => setShowStatusModal(false)} className="px-5 py-2 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-100">
                Cancel
              </button>
              <button onClick={() => {
                  handleTableStatusChange(
